...
После расследования [падения версии 49](android-startup-crash-49-2026-09-08.md) этот gate также проверяет в готовом DEX публичные конструкторы Room-баз WorkManager и Moloco. Сохранённый класс без конструктора недостаточен для reflection в R8 full mode. При новом reflection entry point нужна проверка его фактического вызова в release-коде; успешный `am start -W`, R8 coverage или проверка текста правил не доказывают запуск игры. Проверять сохранение процесса и достижение игрового экрана. APK из Play и локальный APK могут иметь разные подписи; проверять сертификаты до обновления и не обходить несовместимость удалением приложения с данными.
